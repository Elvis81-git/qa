...
const mcqAnswerSetup = document.getElementById('mcq-answer-setup');

gameModeRadios.forEach(radio => {
  radio.addEventListener('change', (e) => {
    playSound('click');
    if (e.target.value === 'mcq') {
      mcqAnswerSetup.classList.remove('hidden');
    } else {
      mcqAnswerSetup.classList.add('hidden');
    }
  });
});
...
startGameBtn.addEventListener('click', () => {
  ...
  const mode = selectedModeEl ? selectedModeEl.value : 'buzzer';
  
  let correctAnswer = [];
  if (mode === 'mcq') {
    const checkedBoxes = document.querySelectorAll('#mcq-answer-setup input[type="checkbox"]:checked');
    correctAnswer = Array.from(checkedBoxes).map(cb => cb.value);
  }
  
  socket.emit('start-game', { mode, correctAnswer });
});
...
// 玩家 MCQ 答案按鈕事件與手寫作答按鈕
const playerWrittenInput = document.getElementById('player-written-input');
const submitWrittenBtn = document.getElementById('submit-written-btn');
const mcqOptBtns = document.querySelectorAll('.mcq-opt-btn');
const submitMcqBtn = document.getElementById('submit-mcq-btn');

// MCQ 選項按鈕點擊效果
mcqOptBtns.forEach(btn => {
...
// 送出問答答案
submitWrittenBtn.addEventListener('click', () => {
  const answer = playerWrittenInput.value.trim();
  if (!answer) {
    alert('請輸入答案內容！');
    return;
  }
  ...
  playerWrittenInput.disabled = true;
  buzzerStatusText.textContent = '作答提交中...';
...
submitMcqBtn.addEventListener('click', () => {
  const selectedOpts = [];
  document.querySelectorAll('.mcq-opt-btn.selected').forEach(btn => {
    selectedOpts.push(btn.getAttribute('data-option'));
  });

  if (selectedOpts.length === 0) {
    if (!confirm('您尚未選擇任何選項，確定要送出空白答案嗎？')) {
      return;
    }
  }
  ...
  mcqOptBtns.forEach(btn => btn.disabled = true);
  buzzerStatusText.textContent = '選項提交中...';

  socket.emit('submit-answer', { answer: selectedOpts });
});
...
function resetBuzzerUI() {
  ...
  // Written 重設
  if (playerWrittenInput) {
    playerWrittenInput.value = '';
    playerWrittenInput.disabled = true;
  }
  if (submitWrittenBtn) {
    submitWrittenBtn.disabled = true;
...
socket.on('game-starting', ({ serverTime, countdownMs, mode }) => {
  currentGameMode = mode || 'buzzer';
  ...
  if (isHost) {
    ...
    buzzerStatusText.textContent = '預備作答...';
    
    // 切換面板顯示
    const buzzerPanel = document.getElementById('player-buzzer-panel');
    const writtenPanel = document.getElementById('player-written-panel');
    const mcqPanel = document.getElementById('player-mcq-panel');
    
    buzzerPanel.classList.add('hidden');
    writtenPanel.classList.add('hidden');
    mcqPanel.classList.add('hidden');
    
    if (currentGameMode === 'buzzer') {
      buzzerPanel.classList.remove('hidden');
    } else if (currentGameMode === 'written') {
      writtenPanel.classList.remove('hidden');
      playerWrittenInput.value = '';
      playerWrittenInput.disabled = true;
      submitWrittenBtn.disabled = true;
    } else if (currentGameMode === 'mcq') {
  ...
  const interval = setInterval(() => {
    ...
    if (elapsed >= duration) {
      ...
      if (isHost) {
        gameStatusBadge.textContent = '作答中！';
        ...
        if (currentGameMode === 'buzzer') {
          ...
          buzzerStatusText.textContent = '請輸入答案並點擊送出！';
          buzzerStatusText.style.color = 'var(--primary-cyan)';
          playerWrittenInput.disabled = false;
          submitWrittenBtn.disabled = false;
          playerWrittenInput.focus();
        } else if (currentGameMode === 'mcq') {
          buzzerStatusText.textContent = '請點選選項並送出！';
          buzzerStatusText.style.color = 'var(--primary-cyan)';
          mcqOptBtns.forEach(btn => btn.disabled = false);
...
socket.on('buzz-updated', (buzzes) => {
  ...
  if (isHost) {
    ...
    if (currentGameMode === 'buzzer') {
      ...
      winnerTimeText.innerHTML = `答案: <span class="answer-highlight">"${winner.answer}"</span><br>反應時間: ${(winner.elapsed / 1000).toFixed(3)} 秒`;
    } else if (currentGameMode === 'mcq') {
      const checkIcon = winner.isCorrect ? '✅ 正確' : '❌ 錯誤';
      const colorClass = winner.isCorrect ? 'text-success' : 'text-danger';
      const selectedStr = Array.isArray(winner.answer) ? winner.answer.join(', ') : '無';
      winnerTimeText.innerHTML = `已選: <span class="answer-highlight">${selectedStr}</span> (<span class="${colorClass}">${checkIcon}</span>)<br>反應時間: ${(winner.elapsed / 1000).toFixed(3)} 秒`;
    }
    
    // 多選題模式下由系統自動評分，隱藏手動審核判定按鈕
    const judgeActions = document.querySelector('.judge-actions');
    if (currentGameMode === 'mcq') {
    ...
    buzzes.slice(1).forEach((buzz, index) => {
      ...
      let extraInfo = '';
      if (currentGameMode === 'written') {
        extraInfo = `<span class="runner-answer">"${buzz.answer}"</span>`;
      } else if (currentGameMode === 'mcq') {
        const checkIcon = buzz.isCorrect ? '✅' : '❌';
        const selectedStr = Array.isArray(buzz.answer) ? buzz.answer.join(', ') : '無';
        extraInfo = `<span class="runner-answer">${selectedStr} (${checkIcon})</span>`;
      }

      timeSpan.innerHTML = `${extraInfo} +${((buzz.elapsed - winner.elapsed) / 1000).toFixed(3)} 秒`;
  ...
  if (!isHost) {
    // 找出自己的作答順序
    const myRankIndex = buzzes.findIndex(b => b.id === socket.id);
    ...
    if (myRankIndex !== -1) {
      ...
      if (currentGameMode === 'buzzer') {
        ...
        if (myRankIndex === 0) {
          playerResultIcon.textContent = '✍️';
          playerResultTitle.textContent = '您是最快送出答案的！';
          playerResultTitle.style.color = 'var(--primary-cyan)';
          playerResultDetail.textContent = `答案內容: "${myRecord.answer}"。請等待房主審核是否正確...`;
          buzzerStatusText.textContent = '答案已送出，等待審核中...';
          buzzerStatusText.style.color = 'var(--primary-cyan)';
        } else {
          playerResultIcon.textContent = '📝';
          playerResultTitle.textContent = `答案已送出！順位為第 ${myRankIndex + 1}`;
          playerResultTitle.style.color = 'var(--text-secondary)';
          playerResultDetail.textContent = `答案內容: "${myRecord.answer}"。前面還有 ${myRankIndex} 位玩家正在審核。`;
          buzzerStatusText.textContent = `答案已送出，順位第 ${myRankIndex + 1}`;
          buzzerStatusText.style.color = 'var(--text-secondary)';
        }
      } else if (currentGameMode === 'mcq') {
        mcqOptBtns.forEach(btn => btn.disabled = true);
        ...
        const selectedStr = Array.isArray(myRecord.answer) ? myRecord.answer.join(', ') : '無';

        if (myRecord.isCorrect) {
          const firstCorrectIndex = buzzes.findIndex(b => b.isCorrect === true);
          const isFirstCorrect = (buzzes[firstCorrectIndex].id === socket.id);

          playerResultIcon.textContent = '🎉';
          playerResultTitle.style.color = 'var(--success-green)';

          if (isFirstCorrect) {
            playerResultTitle.textContent = '答對了！(最快答對 👑)';
            playerResultDetail.textContent = `您的選擇是: [${selectedStr}]。恭喜您成為最快答對的玩家，加 1 分！`;
            buzzerStatusText.textContent = '最快答對！已為您加分！';
            buzzerStatusText.style.color = 'var(--success-green)';
          } else {
            playerResultTitle.textContent = '答對了！';
            playerResultDetail.textContent = `您的選擇是: [${selectedStr}]。答案正確，可惜前面有其他玩家更早答對喔。`;
            buzzerStatusText.textContent = '答對了！';
            buzzerStatusText.style.color = 'var(--success-green)';
          }
        } else {
          playerResultIcon.textContent = '❌';
          ...
          playerResultDetail.textContent = `您的選擇是: [${selectedStr}]。請再接再厲！`;
...
socket.on('game-reset', () => {
  if (isHost) {
    ...
    // 重設模式控制項
    const buzzerRadio = document.querySelector('input[value="buzzer"]');
    if (buzzerRadio) {
      buzzerRadio.checked = true;
    }
    mcqAnswerSetup.classList.add('hidden');
    const checkedBoxes = document.querySelectorAll('#mcq-answer-setup input[type="checkbox"]');
