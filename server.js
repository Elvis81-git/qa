...
app.get('/', (req, res) => {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    res.status(404).send('錯誤：找不到 index.html。請確保 public 資料夾內有 index.html 檔案，且已上傳至 GitHub。您可瀏覽 /debug-dir 查看部署目錄結構。');
  }
});

// 除錯路由，用來檢查 Render 上的檔案結構
app.get('/debug-dir', (req, res) => {
  try {
    ...
    // 同時檢查是否有大小寫差異的 Public 資料夾
    const hasPublicUpper = fs.existsSync(path.join(__dirname, 'Public'));
    let publicUpperFiles = [];
    if (hasPublicUpper) {
      publicUpperFiles = fs.readdirSync(path.join(__dirname, 'Public'));
    }

    res.json({
      __dirname,
      rootFiles,
      hasPublicFolderLower: hasPublic,
      publicFolderLowerFiles: publicFiles,
      hasPublicFolderUpper: hasPublicUpper,
      publicFolderUpperFiles: publicUpperFiles
    });
  } catch (err) {
...
io.on('connection', (socket) => {
  ...
  socket.on('create-room', () => {
    ...
    rooms.set(roomId, {
      ...
      mode: 'buzzer', // buzzer, written, mcq
      correctAnswer: [], // 多選題答案, 如 ['A', 'C']
      activeTime: null,
      buzzes: []
    });
  ...
  // 3. 房主開始倒數
  socket.on('start-game', ({ mode, correctAnswer }) => {
    ...
    room.mode = mode || 'buzzer';
    room.correctAnswer = correctAnswer || [];
    room.activeTime = null;
    room.buzzes = [];

    // 發送通知給所有人開始倒數 3 秒，並傳送當前遊戲模式
    const serverTime = Date.now();
    io.to(roomId).emit('game-starting', { serverTime, countdownMs: 3000, mode: room.mode });
    console.log(`房間 ${roomId} 開始倒數，模式: ${room.mode}`);
    ...
    setTimeout(() => {
      ...
      if (currentRoom && currentRoom.status === 'countdown') {
        ...
        io.to(roomId).emit('buzzer-active', { activeTime: currentRoom.activeTime, mode: currentRoom.mode });
        console.log(`房間 ${roomId} 搶答/作答按鈕已啟用，模式: ${currentRoom.mode}`);
      }
    }, 3000);
  });

  // 4. 玩家按下搶答按鈕 (僅適用於 buzzer 模式)
  socket.on('buzz', () => {
    const roomId = socket.roomId;
    if (!roomId) return;

    const room = rooms.get(roomId);
    if (!room || room.status !== 'active' || room.mode !== 'buzzer') {
      // 若尚未開始或已結束，則不處理
  ...
  // 4.5 玩家送出作答 (適用於 written 與 mcq 模式)
  socket.on('submit-answer', ({ answer }) => {
    ...
    if (room.mode !== 'written' && room.mode !== 'mcq') return;

    const player = room.players.find(p => p.id === socket.id);
    if (!player) return;

    // 檢查此玩家是否已經送出過
    const alreadySubmitted = room.buzzes.some(b => b.id === socket.id);
    if (alreadySubmitted) return;

    const submitTime = Date.now();
    const elapsed = submitTime - room.activeTime;
    ...
    const record = {
      ...
      time: submitTime,
      elapsed: elapsed,
      answer: answer // 可以是字串 (問答) 或 陣列 (選擇題)
    };

    if (room.mode === 'mcq') {
      // 自動比對答案 (複選題，需比對陣列內容)
      const sortedCorrect = Array.isArray(room.correctAnswer) ? [...room.correctAnswer].sort().join(',') : '';
      const sortedAnswer = Array.isArray(answer) ? [...answer].sort().join(',') : '';
      const isCorrect = (sortedCorrect === sortedAnswer);
      
      record.isCorrect = isCorrect;

      // 檢查此人是不是最快答對的玩家
      const hasPreviousCorrect = room.buzzes.some(b => b.isCorrect === true);
      if (isCorrect && !hasPreviousCorrect) {
        player.score += 1;
        // 廣播自動答對的事件與加分
        io.to(roomId).emit('answer-result', { playerId: player.id, name: player.name, result: 'correct' });
        io.to(roomId).emit('players-updated', room.players);
      }
    }

    room.buzzes.push(record);

    // 廣播最新提交狀態
    io.to(roomId).emit('buzz-updated', room.buzzes);
    console.log(`房間 ${roomId} - 玩家 ${player.name} 提交答案，模式: ${room.mode}，反應時間: ${elapsed}ms`);
  });
